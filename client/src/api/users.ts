...
export async function updateUsuario({ id, nome, login, senha, role, cracha, status, token }: UpdateUsuarioParams) {
    return client({ url: '/usuarios', token, method: 'PUT', data: { id, nome, login, senha, role, cracha, status } })
}
